...
char *get_client_ip(int client_fd, const char *buffer) {
    ...
    if (buffer) {
        ...
        if (x_real_ip) {
            ...
            ip[i] = '\0';
            DBhttp_print_debug("Found X-Real-IP: %s", ip);
            return ip;
        }
        ...
        if (x_forwarded_for) {
            ...
            ip[i] = '\0';
            DBhttp_print_debug("Found X-Forwarded-For: %s", ip);
            return ip;
        }
    }
    ...
    if (getpeername(client_fd, (struct sockaddr*)&addr, &addrlen) == 0) {
        if (addr.ss_family == AF_INET) {
            ...
            DBhttp_print_debug("Extracted socket IP: %s", ip);
            return ip;
        }
    }
    
    // Default IP
    DBhttp_print_debug("Using default IP: 127.0.0.2");
